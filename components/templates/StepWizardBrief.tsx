    ...
    <div className={`min-h-screen bg-gradient-to-br ${colors.bg} text-white`}>
          ...
          <div className="space-y-8">
            {steps?.map((step, index) => (
              <div key={index} className="flex items-start space-x-6">
                <div className={`w-12 h-12 bg-gradient-to-r ${colors.accent} rounded-full flex items-center justify-center text-white font-bold text-xl flex-shrink-0`}>
                  {index + 1}
                </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className={`grid ${gridLayout.gridCols} gap-8`}>
            {highlights?.slice(0, gridLayout.maxItems).map((highlight, index) => (
              ...
              >
                <div className="text-4xl mb-4">{getFeatureEmoji(highlight)}</div>
                <h3 className="text-xl font-bold text-white mb-4">{highlight}</h3>
                <p className="text-white/70">
                  Important highlight that makes {title} special.
                </p>
              </div>
            ))}
          </div>
      ...
      {disclaimer && (
        <section className="py-8 px-6">
          <div className="max-w-6xl mx-auto">
            <div className="bg-yellow-500/10 backdrop-blur border border-yellow-500/30 rounded-2xl p-6 text-center">
              <p className="text-yellow-300 text-sm">{disclaimer}</p>
            </div>
          </div>
        </section>
      )}
